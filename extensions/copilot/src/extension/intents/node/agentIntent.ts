...
export class AgentIntent extends EditCodeIntent {
	...
	getOrCreateBackgroundSummarizer(sessionId: string, modelMaxPromptTokens: number, endpointModel: string): BackgroundSummarizer {
		let summarizer = this._backgroundSummarizers.get(sessionId);
		if (summarizer && summarizer.endpointModel !== endpointModel) {
			// Endpoint switched mid-session. A summary kicked off against the
			// previous model's prefix would be applied unconditionally on the
			// next pre-render, producing a surprising "Compacted conversation"
			// notice on the new model — cancel and start fresh.
			summarizer.cancel();
			this._backgroundSummarizers.delete(sessionId);
			summarizer = undefined;
		}
		if (!summarizer) {
			summarizer = new BackgroundSummarizer(modelMaxPromptTokens, endpointModel);
...
export class AgentIntentInvocation extends EditCodeIntentInvocation implements IIntentInvocation {
	...
	override async buildPrompt(
		...
		// switch to a larger window (or a lowered context-size override) the
		// ratio drops below `applyMinRatio`; applying a stale summary in that
	...
	private _getOrCreateBackgroundSummarizer(sessionId: string | undefined): BackgroundSummarizer | undefined {
		...
		return this.intent.getOrCreateBackgroundSummarizer(sessionId, this.endpoint.modelMaxPromptTokens, this.endpoint.model);
	}
